...
/*          position: {
    ...
    //$jquery_aleksi('#ui-tab-dialog-close').append($jquery_aleksi('a.ui-dialog-titlebar-close'));
    //$jquery_aleksi('#ui-tab-dialog-close').append($jquery_aleksi('#closer'));
    ...
    //$jquery_aleksi('#aleksi_dialog').addClass('ui-tabs')
    //               .prepend($jquery_aleksi('#aleksi_tabs'))
    //               .draggable('option','handle','#aleksi_tabs'); 
    //$jquery_aleksi("#aleksi_dialog").find(".ui-dialog-titlebar").remove();
    $jquery_aleksi('#aleksi_tabs').addClass('ui-dialog-titlebar');
   // $jquery_aleksi( "#closer" ).click(function() {
   // });
...
//    });
    //$jquery_aleksi("#open_config_dialog_button").button();
    ...
    $jquery_aleksi("#open_session_dialog_button").on('click', function(ev) {
        //$jquery_aleksi("#session_dialog").dialog("option", "position", [e.pageX, e.pageY]);
        $jquery_aleksi("#config_dialog").dialog('close');
    ...
    $jquery_aleksi("#open_quizlet_dialog_button").on('click', function(ev) {
        //$jquery_aleksi("#quizlet_dialog").dialog("option", "position", [e.pageX, e.pageY]);
        $jquery_aleksi("#config_dialog").dialog('close');
    ...
    $jquery_aleksi( "a" ).on('click', function(e) {
      //if ($jquery_aleksi.contains(document.getElementById("aleksi_dialog"),e.target) || $jquery_aleksi.contains(document.getElementById("navbar"),e.target) || $jquery_aleksi.contains(document.getElementById("session_dialog"),e.target) || $jquery_aleksi.contains(document.getElementById("quizlet_dialog"),e.target) ) {
    ...
    if (!isMobile){
      ...
      $jquery_aleksi("#aleksi").click(function(e) {
        e.preventDefault();
        e.stopPropagation();
        $jquery_aleksi('#aleksi_dialog').dialog('close');
      });
...
//        data    : {'word': word}, 
	data : {'lang': lang},
        ...
        beforeSend: function(msg){
            //$jquery_aleksi( "#aleksi" ).tabs();
            //$jquery_aleksi( "#aleksi_dialog" ).dialog('option', 'title', word);
            if (!isMobile){
              if ($jquery_aleksi(".ui-widget-overlay")) //the dialog has popped up in modal view
              {
                  //fix the overlay so it scrolls down with the page
            ...
            //$jquery_aleksi('a[href="#aleksi_main"]').click();
        ...
        success : function(response){
            //var analysis_results = $jquery_aleksi.parseJSON(response);
...
//                    pin_icon_span.attr("style", "display:inline-block; direction:rtl;");
        ...
        successHTML : function(data){
            var aleksi_word = $jquery_aleksi(data).find('#aleksi_word').text();
            var aleksi_main = $jquery_aleksi(data).find('#aleksi_main');
            $jquery_aleksi( "#aleksi_main" ).replaceWith(aleksi_main);
            $jquery_aleksi( "#aleksi_word" ).text(aleksi_word);
            $jquery_aleksi( ".radio_input" ).checkboxradio();
            build_pins_table();
            $jquery_aleksi('a[href="#aleksi_main"]').click();
        },
        */
...
$jquery_aleksi(function() {
  ...
  function getFullWord(event) {
    ...
    //if (!textNode || textNode.nodeType !== Node.TEXT_NODE || $jquery_aleksi.contains(document.getElementById("aleksi_dialog").parentNode, textNode) || $jquery_aleksi.contains(document.getElementById("navbar").parentNode, textNode) || $jquery_aleksi("#session_dialog").dialog('isOpen') || $jquery_aleksi("#config_dialog").dialog('isOpen')) {
  ...
  $jquery_aleksi(document).click(function(e) {
...
function build_pins_table ()
{
    ...
    for (var i = pins.length-1; i >= 0; i--) {
      ...
      //var unpin_div = $jquery_aleksi(document.createElement("div"));
      //var site_div = $jquery_aleksi(document.createElement("div"));
      ...
      var pin = pins[i];
      /*if (pin.website.id==session.website.id) {
        site_link.attr("href", "javascript:void(0);");
      } else {
        site_link.attr("href", "javascript:set_website("+pins[i].website.id+");");
      }*/
      if (pin.website) {
      ...
      //unpin_div.append(unpin_link)
      //site_div.append(site_link);
    ...
    //$jquery_aleksi("#aleksi_pins_table_div").find("table").replaceWith(table);
}
function set_website(new_website_id){
    if (session.website.id!=new_website_id) {
        jQuery.ajax({
            ...
            success : function(response){
                location.reload();
                //$jquery_aleksi(window).html(response);
            }    
        });
    }
}
...
//  $jquery_aleksi("#associated_website").append(session.websites[i].title);
        }
...
function build_quizlet_set_selector ()
{
    ...
    if (use_dropdown_list) {
        ...
        //var label = $jquery_aleksi(document.createElement("label"));
        //label.attr("id", "quizlet_set_selectmenu_label");
        //label.attr("for", "quizlet_set_selectmenu");
        //label.append("Associated Quizlet set:");
        var option = $jquery_aleksi(document.createElement("option"));
        ...
        //quizlet_set_selector.append(label);
...
function get_pins ()
{
    $jquery_aleksi.ajax({
      ...
      'success': function(pins)
      {
        ...
        update_create_quizlet_set_state();
//        update_sync_to_quizlet_state();
        //var data = $jquery_aleksi.parseJSON(response);
        //response['pins'].forEach( function(fi) {
      },
/*      beforeSend: function()
      {
//      $jquery_aleksi(document).ready(function () {
//        $jquery_aleksi("#aleksi_pinned_status").text("Transferring fi data to Quizlet...");
//      });
      },
      */
      'error': function(data)
...
function save_pins (url)
{
    ...
    var set_id = $jquery_aleksi("input[name=set_id]:checked").val();
    var new_set_title = $jquery_aleksi("input[name=new_set_title]").val();
    var new_set_description = $jquery_aleksi("input[name=new_set_description]").val();
    $jquery_aleksi.ajax({
      'url': url,
      'type': 'POST',
      'dataType': 'json', 
      ...
      'success': function(data)
      {
        clear_pins();
        reset_ui();
        $jquery_aleksi("#aleksi_pinned_status").text("Term data transferred to Quizlet study set!");
      },
      beforeSend: function()
      {
//      $jquery_aleksi(document).ready(function () {
...
/*
  $jquery_aleksi( "#website_selector" ).controlgroup({
    "direction": "horizontal"
  });
  $jquery_aleksi( "#set_website_url" ).controlgroup({
    "direction": "horizontal"
  });
*/
...
//$jquery_aleksi('input:radio[name="website_setter"]:checked').change(function () {
            if (this.value == 'set_url') {
                //$jquery_aleksi("#website_url_input").show()
                //$jquery_aleksi("#website_selector").hide()
                //$jquery_aleksi("input#website_url_input").removeAttr("disabled");
                //$jquery_aleksi("select#website_selector").attr({"disabled": "disabled"});
                /*$jquery_aleksi("#website_selector").controlgroup();
                $jquery_aleksi("#set_website_url").controlgroup();
                $jquery_aleksi("#website_selector").controlgroup("disable");
                $jquery_aleksi("#set_website_url").controlgroup("enable");
                */
                $jquery_aleksi("#set_website_url").removeClass("ui-state-disabled");
                $jquery_aleksi("#website_selector").addClass("ui-state-disabled");
            }
            //if ($jquery_aleksi("input[name='website_setter']:checked").val() == 'select_active') {
            if (this.value == 'select_active') {
                //$jquery_aleksi("#website_url_input").hide()
                //$jquery_aleksi("#website_selector").show()
                //$jquery_aleksi("input#website_url_input").attr({"disabled": "disabled"});
                //$jquery_aleksi("select#website_selector").removeAttr("disabled");
                /*
                $jquery_aleksi("#website_selector").controlgroup();
                $jquery_aleksi("#set_website_url").controlgroup();
                $jquery_aleksi("#website_selector").controlgroup("enable");
                $jquery_aleksi("#set_website_url").controlgroup("disable");
                */
                $jquery_aleksi("#set_website_url").addClass("ui-state-disabled");
                $jquery_aleksi("#website_selector").removeClass("ui-state-disabled");
            }
  });
  var page_overlay = $jquery_aleksi('<div id="aleksi_overlay"><img src="'+loading_spinner_url+'"/> <p id="aleksi_overlay_msg">Processing</p></div>');
...
/*
function update_sync_to_quizlet_state () {
    if (pins.length>0) {
        $jquery_aleksi("#sync_to_quizlet_button").prop("disabled", false);
        $jquery_aleksi("#sync_to_quizlet_button").button("enable");
    } else {
        $jquery_aleksi("#sync_to_quizlet_button").prop("disabled", true);
        $jquery_aleksi("#sync_to_quizlet_button").button("disable");
    }
}
*/
...
function reset_ui ()
{
    //$jquery_aleksi("#leave_session_button").button();
    ...
    //$jquery_aleksi("#update_website_button").button();
    update_create_quizlet_set_state();
//    update_sync_to_quizlet_state();
    //build_pins_table();
    //get_quizlet_sets();
  ...
  var quizlet_connect = new QuizletConnect(quizlet_auth_url, get_quizlet_sets);
...
//        data    : JSON.stringify({'session_title': session_title , 'session_id': session_id, 'quizlet_set_id': quizlet_set_id}), 
        ...
        success : function(shared_session){
          var load_shared_session_url = window.load_shared_session_url.replace("__shared_session_hash",shared_session.hash);
          $jquery_aleksi("#shared_session_link").attr("href", load_shared_session_url);
          $jquery_aleksi("#share_session_button").hide();
...
function sync_to_quizlet(){
    //var prune_on_sync = $jquery_aleksi("input[name=prune_on_sync]").val();
    ...
    $jquery_aleksi.ajax({
      'url': sync_to_quizlet_url,
      'type': 'POST',
      'tryCount': 0,
      'retryLimit': 3,
      'dataType': 'json', 
      //'data'    : JSON.stringify({'prune_on_sync': prune_on_sync}), 
...
//        update_sync_to_quizlet_state();
      ...
      'error': function(xhr, textStatus, errorThrown) {
        ...
        // handle error
          ...
          var quizlet_connect = new QuizletConnect(quizlet_auth_url, ajax_retry_callback);
