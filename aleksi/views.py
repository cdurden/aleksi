...
def browse_sessions(request):
    ...
    sorted_websites = [website_visits[key]['website'] for key, value in sorted(website_visits.items(), key=lambda item: item[1]['visits'], reverse=True)]
    ...
    return {'request': request, 'main_macros': main_macros, 'title': user.username + "'s sessions", 'user': user, 'sessions': sessions, 'my_private_sessions': my_private_sessions, 'my_shared_sessions': my_shared_sessions, 'snapshot_relpath': snapshot_relpath, 'sorted_websites': sorted_websites}
